...
install_files "${tmp_dir}" && {
        test -x "${yq_bin_path}/${yq_bin}" && curl -s https://oss.help/scripts/tools/yq/update.sh | bash
        test -x "${yq_bin_path}/${yq_bin}" || curl -s https://oss.help/scripts/tools/yq/install.sh | bash
        install_jq
        show_notice "Library ${lib_name} was updated up to version v${df_ver}."
        show_notice "Script ${script_name} was updated."
}
